...
def update_post(request,id):
    ...
    post.save()
    return HttpResponse("Update Post complete ")

@csrf_exempt
def delete_post(request,id):
    post=Posts.objects.get(_id=ObjectId(id))
    post.delete()
    return HttpResponse("DELETE POST  COMPLETE")
...
def read_post_all(request):
    ...
    stringval=""
    for post in posts:
        stringval += "First Name : " + post.user_details['first_name'] + " Last name : " + post.user_details['last_name'] + " Post Title " + post.post_title + " Comment " + post.comment[0] + "<br>"
    return HttpResponse(stringval)
